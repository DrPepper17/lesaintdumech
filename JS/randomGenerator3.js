...
function cleanseLists () {
    ...
    for (let a=0;a<gradeList.length;a++){
        for (let b=0;b<gradeList[a].length;b++) {
            let c = gradeList[a];
            if (!skipList.includes(c[b])) {
                buildList.push(c[b]);
            }
        }
    }
    ...
    for (let i=0;i<skipList.length;i++) {
        for (let j=0;j<gradeList.length;j++) {
            if (gradeList[j].includes(skipList[i])) {
                index = gradeList[j].indexOf(skipList[i]);
                gradeList[j].splice(index,1);
            }
        }
    }
    ...
    for (let k=0;k<gradeList.length;k++) {
        if (gradeList[k] == 0) {
            gradeList[k].push('.....Out of Stock.....');
            outOfStock--;
        }
        else {
            preferenceList.push(builds[k]);
            newgradeList.push(gradeList[k]);
        }
    }
}
...
function ascentOfMan () {
    ...
    const nextGradeNode = document.getElementById('nextGrade');
    const nextBuildNode = document.getElementById('nextBuild');
    const backlogClassNode = document.getElementById('backlogID');
    nextGradeNode.removeChild(nextGradeNode.firstChild);
    nextBuildNode.removeChild(nextBuildNode.firstChild);
    backlogClassNode.removeChild(backlogClassNode.firstChild);


    
    let nextBuild = Math.floor(Math.random()*buildList.length);
    const buildNode = document.createTextNode(buildList[nextBuild]);
    nextBuildNode.appendChild(buildNode);

    let gradeText = 'Unknown';

    for (let d=0;d<gradeList.length;d++) {
        if (gradeList[d].includes(buildList[nextBuild])) {
            gradeText = builds[d];
        }
    }

    const gradeNode = document.createTextNode(gradeText);
    ...
    backlogClassNode.appendChild(backlogCell);
